...
#define GPIO_ISDBT_IRQ 8
#define GPIO_ISDBT_PWR_EN 74
#define GPIO_ISDBT_RST 65

static DEFINE_MUTEX(ringbuffer_lock);

void tunerbb_drv_set_user_stop(int ustop)
{
    user_stop_called = ustop;
}

int get_ustop_state(void)
{
    int ret = 0;
    ret = (user_stop_called == 1) ? 1 : 0;
...
u8 isdbt_hw_init(void)
{
    ...
    PRINTF(0, "[1seg][MTK] hwPowerOn MT6323_POWER_LDO_VGP3(%d)\n", ret);
    msWait(2);
    ...
    mt_set_gpio_mode(GPIO80|0x80000000, GPIO_MODE_01);    //GPIO_MATV_I2S_WS_PIN_M_SPI_CS   GPIO_MODE_01
...
void isdbt_hw_deinit(void)
{
    ...
    //SPI CS
    mt_set_gpio_mode(GPIO80|0x80000000, GPIO_MODE_00); //GPIO_MATV_I2S_WS_PIN_M_GPIO
    mt_set_gpio_pull_enable(GPIO80|0x80000000, GPIO_PULL_ENABLE);
    mt_set_gpio_pull_select(GPIO80|0x80000000,GPIO_PULL_DOWN);
    ...
    PRINTF(0, "[1seg][MTK] hwPowerDown MT6323_POWER_LDO_VGP3(%d)\n", ret);
#else //CONFIG_ARCH_MT6582
...
int isdbt_open (struct inode *inode, struct file *filp)
{
    ...
    //enable irq
    //mt_eint_unmask(CUST_EINT_DTV_NUM);
    PRINTF(hInit, "[1seg] isdbt file descriptor open2 mt_eint_unmask\n");
/*
...
#if 0
    buf = (void*)((unsigned long)puserdata->data_buf_addr) + sizeof(DMB_BB_HEADER_TYPE);
    count = puserdata->data_buf_size - sizeof(DMB_BB_HEADER_TYPE);
    count = (count/188)*188;
#endif

    buf = (void*)((unsigned long)puserdata->data_buf_addr);
...
void isdbt_get_signal_info(HANDLE hDevice,u16 *lock, u32 *ui32BER, u32 *ui32PER, s32 *rssi, u16 *cn, u16 *agc)
{
    ...
    BBM_READ(hDevice, BBM_BUF_OVERRUN, &bufOverRun);
    ...
    *rssi = abs(dm.rssi) * 100;
    *cn = dm.wscn * 100;

    PRINTF(hDevice, "[1seg][FC8150] LOCK :%d, BER: %d, PER : %d, RSSI : %d, CN : %d, agc : %d, bufIntStatus: %d, bufOverRun: 0x%x\n", *lock, *ui32BER, *ui32PER, *rssi, *cn, *agc, bufIntStatus, bufOverRun);
...
long isdbt_ioctl (struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case LGE_BROADCAST_DMB_IOCTL_USER_STOP:
            //hOpen->isdbttype = 0;     // 20150813 TD 203195 work around patch
            //uData = *argp;
            err = copy_from_user((void *)&uData, (void *)arg, size);
            PRINTF(0, "[1seg] broadcast_dmb_user_stop (%d)\n", uData);
            tunerbb_drv_set_user_stop(uData);
            res = BBM_OK;
            break;
        ...
        case LGE_BROADCAST_DMB_IOCTL_GET_SIG_INFO:
            {
                ...
                if(copy_from_user(&udata, argp, sizeof(struct broadcast_dmb_control_info)))
                {
                ...
                switch(udata.cmd_info.cmd) {
                    case 1: /* all */
                        udata.sig_info.info.mmb_info.cn = (int)isdbt_signal_info.cn;
                        udata.sig_info.info.mmb_info.ber_a = (int)isdbt_signal_info.ber;
                        udata.sig_info.info.mmb_info.per_a = (int)isdbt_signal_info.per;
                        udata.sig_info.info.mmb_info.layerinfo_a = 1;
                        udata.sig_info.info.mmb_info.tmccinfo = 0xFF;
                        udata.sig_info.info.mmb_info.receive_status = 0xFF;
                        udata.sig_info.info.mmb_info.rssi = (int)isdbt_signal_info.rssi;
                        udata.sig_info.info.mmb_info.scan_status = 0xFF;
                        udata.sig_info.info.mmb_info.sysinfo = 0xFF;
                        udata.sig_info.info.mmb_info.total_tsp_a = (int)isdbt_signal_info.TotalTSP;

                        udata.sig_info.info.mmb_info.ber_b = 100000;
                        udata.sig_info.info.mmb_info.per_b = 100000;
                        udata.sig_info.info.mmb_info.layerinfo_b = 0xFFFF;
                        udata.sig_info.info.mmb_info.total_tsp_b = 0;

                        udata.sig_info.info.mmb_info.ber_c = 100000;
                        udata.sig_info.info.mmb_info.per_c = 100000;
                        udata.sig_info.info.mmb_info.layerinfo_c = 0xFFFF;
                        udata.sig_info.info.mmb_info.total_tsp_c = 0;

                        udata.sig_info.info.mmb_info.antenna_level_fullseg = 0;
                        udata.sig_info.info.mmb_info.antenna_level_oneseg = isdbt_get_antenna_level(isdbt_signal_info.ber, isdbt_signal_info.cn, udata.sig_info.info.mmb_info.antenna_level_oneseg);
                        udata.sig_info.info.mmb_info.agc = (int)isdbt_signal_info.agc;
                        udata.sig_info.info.mmb_info.ber_1seg = (int)isdbt_signal_info.ber;
                        udata.sig_info.info.mmb_info.per_1seg = (int)isdbt_signal_info.per;
                        udata.sig_info.info.mmb_info.total_tsp_1seg = (int)isdbt_signal_info.TotalTSP;
                        udata.sig_info.info.mmb_info.err_tsp_1seg = (int)isdbt_signal_info.ErrTSP;
                        udata.sig_info.info.mmb_info.ber_fullseg = 100000;
                        udata.sig_info.info.mmb_info.per_fullseg = 100000;
                        udata.sig_info.info.mmb_info.total_tsp_fullseg = 0;
                        udata.sig_info.info.mmb_info.err_tsp_fullseg = 0;
			  
                        break;

                    case 12: /* oneseg_sig_info */
                        udata.sig_info.info.oneseg_info.lock = (int)isdbt_signal_info.lock;
                        udata.sig_info.info.oneseg_info.cn = (int)isdbt_signal_info.cn;
                        udata.sig_info.info.oneseg_info.ber = (int)isdbt_signal_info.ber;
                        udata.sig_info.info.oneseg_info.per = (int)isdbt_signal_info.per;
                        udata.sig_info.info.oneseg_info.agc = (int)isdbt_signal_info.agc;
                        udata.sig_info.info.oneseg_info.rssi = (int)isdbt_signal_info.rssi;
                        udata.sig_info.info.oneseg_info.ErrTSP = (int)isdbt_signal_info.ErrTSP;
                        udata.sig_info.info.oneseg_info.TotalTSP = (int)isdbt_signal_info.TotalTSP;
                        udata.sig_info.info.mmb_info.antenna_level_oneseg = isdbt_get_antenna_level(isdbt_signal_info.ber, isdbt_signal_info.cn, udata.sig_info.info.mmb_info.antenna_level_oneseg);
                        udata.sig_info.info.oneseg_info.Num = 0;
                        udata.sig_info.info.oneseg_info.Exp = 0;
                        udata.sig_info.info.oneseg_info.mode = 0;
                        break;
                }
                
                if(copy_to_user((void *)argp, &udata, sizeof(struct broadcast_dmb_control_info)))
                {
        ...
        case LGE_BROADCAST_DMB_IOCTL_GET_DMB_DATA:
            //PRINTF(0, "[1seg] LGE_BROADCAST_DMB_IOCTL_GET_DMB_DATA \n");
...
int isdbt_init(void)
{
    ...
    PRINTF(hInit, "[1seg] g_lge_dtv_flag : [%d] result : [%d]\n", g_lge_dtv_flag, result);
    if(result == 1)
    {
        PRINTF(hInit, "[1seg] L80 PLUS DTV support model!!!\n");
#endif defined(CONFIG_ARCH_MT6582)// MediaTek platform
        PRINTF(hInit, "[1seg] isdbt_init DRV V1p12 20140611\n");
        ...
        PRINTF(0,"[1seg][MTK] GPIO_1SEG_INT mt_eint_set_sens = %d!!!\n", ret);
        mt_eint_set_hw_debounce(CUST_EINT_DTV_NUM, CUST_EINT_DTV_DEBOUNCE_CN);
        PRINTF(0,"[1seg][MTK] GPIO_1SEG_INT mt_eint_set_hw_debounce !!!\n");
        ...
        PRINTF(0,"[1seg][MTK] GPIO_1SEG_INT mt_eint_registration!!!\n");
        ...
        // Power OFF
        hwPowerDown(MT6323_POWER_LDO_VGP3, "1V8_MTK_DTV");    // IO power Off
        PRINTF(0, "[1seg][MTK][init2] hwPowerDown MT6323_POWER_LDO_VGP3\n");
