...
64,
64,
IMX135MIPI_PV_LINE_LENGTH_PIXELS,
IMX135MIPI_PV_FRAME_LENGTH_LINES,
IMX135MIPI_VIDEO_LINE_LENGTH_PIXELS,
IMX135MIPI_VIDEO_FRAME_LENGTH_LINES,
IMX135MIPI_FULL_LINE_LENGTH_PIXELS,
IMX135MIPI_FULL_FRAME_LENGTH_LINES,
0,
0,
0,
0,
...
kal_uint32 capture_shutter_imx135 = 1;
kal_uint32 capture_gain_imx135 = 64;																				 // Gain Index
...
void IMX135MIPI_write_shutter(kal_uint16 shutter)
{
	...
	unsigned long flags;
	SENSORDB("IMX135MIPI_write_shutter shutter = %d Autoflicker(%d)\n",shutter, IMX135MIPI_Auto_Flicker_mode);
    if (IMX135MIPI_sensor.pv_mode == KAL_TRUE) 
	 ...
	 if (IMX135MIPI_sensor.pv_mode == KAL_TRUE) 
	 {
	   ...
	   framerate = (10 * IMX135MIPI_sensor.pv_pclk) / (frame_length * line_length);


	   //if(IMX135MIPI_Auto_Flicker_mode == KAL_TRUE)
	   {
		 if(framerate>=280)
		  {
			 framerate=280;
			 frame_length = (10 * IMX135MIPI_sensor.pv_pclk) / (framerate * line_length);
		  }
	   ...
	   SENSORDB("wwww [IMX135MIPI]enter IMX135MIPI_write_shutter flicker(%d) frame(%d)\n",IMX135MIPI_Auto_Flicker_mode,framerate); 
	 ...
	 else if (IMX135MIPI_sensor.video_mode== KAL_TRUE) 
     {
		...
		framerate = (10 * IMX135MIPI_sensor.video_pclk) / (frame_length * line_length);
		...
		//if(IMX135MIPI_Auto_Flicker_mode == KAL_TRUE)
		{
		  if(framerate>=280)
		   {
			  framerate=280;
			  frame_length = (10 * IMX135MIPI_sensor.video_pclk) / (framerate * line_length);
		   }
		  else if(framerate>=220&&framerate<280)
		   {
			  framerate=220;
		...
		SENSORDB("[IMX135MIPI]enter IMX135MIPI_write_shutter flicker(%d) frame(%d)\n",IMX135MIPI_Auto_Flicker_mode,framerate); 
		...
		framerate = (10 * IMX135MIPI_sensor.cp_pclk) / (frame_length * line_length);
		...
		//if(IMX135MIPI_Auto_Flicker_mode == KAL_TRUE)
		{
		  if(framerate>=280)
		   {
			  framerate=280;
			  frame_length = (10 * IMX135MIPI_sensor.cp_pclk) / (framerate * line_length);
		   }
		  else if(framerate>=220&&framerate<280)
		   {
			  framerate=220;
...
void set_IMX135MIPI_shutter(kal_uint16 iShutter)
{

	unsigned long flags;
	
#ifdef IMX091MIPI_DRIVER_TRACE
	SENSORDB("set_IMX135MIPI_shutter:%x \n",iShutter);
#endif

   
    if((IMX135MIPI_sensor.pv_mode == KAL_FALSE)&&(IMX135MIPI_sensor.is_zsd == KAL_FALSE))
    	{
    	   SENSORDB("[set_IMX135MIPI_shutter]now is in 1/4size cap mode\n");
    	   //return;
    	}
	else if((IMX135MIPI_sensor.is_zsd == KAL_TRUE)&&(IMX135MIPI_sensor.is_zsd_cap == KAL_TRUE))
		{
			SENSORDB("[set_IMX135MIPI_shutter]now is in zsd cap mode\n");

			//SENSORDB("[set_IMX091MIPI_shutter]0x3500:%x\n",IMX091MIPI_read_cmos_sensor(0x3500));
			//SENSORDB("[set_IMX091MIPI_shutter]0x3500:%x\n",IMX091MIPI_read_cmos_sensor(0x3501));
			//SENSORDB("[set_IMX091MIPI_shutter]0x3500:%x\n",IMX091MIPI_read_cmos_sensor(0x3502));
			//return;
		}

/*	if(IMX091MIPI_sensor.shutter == iShutter)
		{
			SENSORDB("[set_IMX091MIPI_shutter]shutter is the same with previous, skip\n");
			return;
		}*/

	//spin_lock_irqsave(&imx135_drv_lock,flags);
	//IMX135MIPI_sensor.shutter = iShutter;
	//spin_unlock_irqrestore(&imx135_drv_lock,flags);
	
    IMX135MIPI_write_shutter(iShutter);

}   /*  Set_IMX091MIPI_Shutter */
void set_IMX135MIPI_shutter_capture(kal_uint16 iShutter)
{

	unsigned long flags;
	
	SENSORDB("set_IMX135MIPI_shutter_capture:%x \n",iShutter);


    
    if((IMX135MIPI_sensor.pv_mode == KAL_FALSE)&&(IMX135MIPI_sensor.is_zsd == KAL_FALSE))
    	{
    	   SENSORDB("[set_IMX091MIPI_shutter]now is in 1/4size cap mode\n");
    	   //return;
    	}
	else if((IMX135MIPI_sensor.is_zsd == KAL_TRUE)&&(IMX135MIPI_sensor.is_zsd_cap == KAL_TRUE))
		{
			SENSORDB("[set_IMX091MIPI_shutter]now is in zsd cap mode\n");
...
/*	if(IMX091MIPI_sensor.shutter == iShutter)
	...
	spin_lock_irqsave(&imx135_drv_lock,flags);
//	IMX091MIPI_sensor.shutter = iShutter;
	spin_unlock_irqrestore(&imx135_drv_lock,flags);
...
kal_uint16 IMX135MIPI_SetGain_capture(kal_uint16 iGain)
{
   kal_uint16 iReg;
   unsigned long flags;
  // 	  return 0;

   SENSORDB("IMX091MIPI_SetGain_capture:%x;\n",iGain);


    

/*   if(IMX091MIPI_sensor.gain == iGain)
   	{
   		SENSORDB("[IMX091MIPI_SetGain]:gain is the same with previous,skip\n");
	 	return 0;
   	}*/

	//spin_lock_irqsave(&imx091mipi_drv_lock,flags);
	//IMX091MIPI_sensor.gain = iGain;
	//spin_unlock_irqrestore(&imx091mipi_drv_lock,flags);
	...
	//imx123 max gain = 8X  
	//IMX091MIPI_write_cmos_sensor(0x0104, 1);  
	IMX135MIPI_write_cmos_sensor(0x205, iReg&0xFF);//only use 0x350b for gain control
	//IMX091MIPI_write_cmos_sensor(0x0104, 0);  
...
void update_capture_shutter_gain_imx135()
{
		
	SENSORDB("!!!update_capture_3A:capture_shutter=0x%x, capture_gain=0x%x \n",capture_shutter_imx135, capture_gain_imx135);
	IMX135MIPI_SetGain_capture(capture_gain_imx135); // change the order for fast mode sequence.
	set_IMX135MIPI_shutter_capture(capture_shutter_imx135);
	//IMX135MIPI_SetGain_capture(capture_gain);
}
...
//quality
   ...
   // ARNR Parameter setting
    ...
    IMX135MIPI_sensor.is_zsd = KAL_FALSE;  //for zsd full size preview
	IMX135MIPI_sensor.is_zsd_cap = KAL_FALSE;
	spin_unlock(&imx135_drv_lock);
...
{	
  ...
  //PLL setting 
	...
	IMX135MIPI_write_cmos_sensor(0x3313,0x18);//   
	IMX135MIPI_write_cmos_sensor(0x331C,0x04);//   
	//IMX135MIPI_write_cmos_sensor(0x331C,0x00);//   Sony Korea 20141001
	IMX135MIPI_write_cmos_sensor(0x331D,0xAB);//   
	//IMX135MIPI_write_cmos_sensor(0x331D,0x52);//   Sony Korea 20141001
	IMX135MIPI_write_cmos_sensor(0x4084,0x00);//   
	IMX135MIPI_write_cmos_sensor(0x4085,0x00);//   
	IMX135MIPI_write_cmos_sensor(0x4086,0x00);//   
	IMX135MIPI_write_cmos_sensor(0x4087,0x00);//   
...
{	
	...
	//IMX135MIPI_write_cmos_sensor(0x331C,0x01);//   Sony Korea 20141001
	IMX135MIPI_write_cmos_sensor(0x331D,0x77);//   
	//IMX135MIPI_write_cmos_sensor(0x331D,0x68);//   Sony Korea 20141001
	IMX135MIPI_write_cmos_sensor(0x4084,0x00);//   
	IMX135MIPI_write_cmos_sensor(0x4085,0x00);//   
	IMX135MIPI_write_cmos_sensor(0x4086,0x00);//   
	IMX135MIPI_write_cmos_sensor(0x4087,0x00);//   
	...
	// integration time setting
#if fast_mode	
    update_capture_shutter_gain_imx135();
#else
	IMX135MIPI_write_cmos_sensor(0x0202,0x0C);//
	IMX135MIPI_write_cmos_sensor(0x0203,0x46);//
    
	//gain setting
	IMX135MIPI_write_cmos_sensor(0x0205,0x00);// mask line1370 commented by Yan.Xu 20140901 on OCS  
#endif	
	IMX135MIPI_write_cmos_sensor(0x020E,0x01);//   
	IMX135MIPI_write_cmos_sensor(0x020F,0x00);//   
	IMX135MIPI_write_cmos_sensor(0x0210,0x01);//   
...
#endif	
#if fast_mode
  IMX135MIPI_write_cmos_sensor(0x3A43,0x00);// <== Mode Clear
#endif  
	IMX135MIPI_write_cmos_sensor(0x0100,0x01);//STREAM START
...
void IMX135MIPISetFlipMirror(kal_int32 imgMirror)
{
    ...
    SENSORDB("IMX135MIPISetFlipMirror=%d\n",imgMirror);
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    switch (ScenarioId)
    {
        ...
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
            ...
            break;
        case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
			if(IMX135MIPI_sensor.is_zsd == KAL_TRUE)
				{
					spin_lock(&imx135_drv_lock);
					IMX135MIPI_sensor.is_zsd_cap = KAL_TRUE;
					spin_unlock(&imx135_drv_lock);
					SENSORDB("IMX135MIPIControl£¬is_zsd_cap is true!\n");
				}
			else
				{
					spin_lock(&imx135_drv_lock);
					IMX135MIPI_sensor.is_zsd_cap = KAL_FALSE;
					spin_unlock(&imx135_drv_lock);
					SENSORDB("IMX135MIPIControl£¬is_zsd_cap is false!\n");
				}
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_SET_ESHUTTER:
            ...
            {
            	SENSORDB("shutter&gain in capture by hhl:IMX135MIPI_SetShutter in feature ctrl\n"); 
            }
#else
            IMX135MIPI_SetShutter(*pFeatureData16);
            SENSORDB("shutter&gain test by hhl:IMX135MIPI_SetShutter in feature ctrl\n"); 
        ...
        case SENSOR_FEATURE_SET_GAIN:
            ...
            {
                SENSORDB("shutter&gain in capture by hhl:IMX135MIPI_SetGain in feature ctrl\n"); 
            }
#else
            IMX135MIPI_SetGain((UINT16) *pFeatureData16);
#endif
	    ...
	    case SENSOR_FEATURE_SET_SENSOR_SYNC:
#if fast_mode
			capture_shutter_imx135 = *(pFeatureData32+2) & 0xffff;
			capture_gain_imx135 = (*(pFeatureData32+2) & 0xffff0000)>>16;
			SENSORDB("\ncapture_shutter 1 :%x \n",capture_shutter_imx135);            
            capture_shutter_imx135= (capture_shutter_imx135 * 19770)/21050;
            SENSORDB("\ncapture_shutter 2 :%x \n",capture_shutter_imx135);
			SENSORDB("capture_gain:%x \n",capture_gain_imx135);
            
#else
            SENSORDB("Not support SYNC in Normal mode \n");
#endif
			break;
       //[LGE_UPDATE_S][yonghwan.lym@lge.com][2014-07-10] Get FOV from sensor for CTS
        ...
        //[LGE_UPDATE_E][yonghwan.lym@lge.com][2014-07-10] Get FOV from sensor for CTS
        default:
            break;
    }
    return ERROR_NONE;
