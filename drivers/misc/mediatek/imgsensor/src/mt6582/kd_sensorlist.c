...
int module_flag = -1; // 1:Sunny ov8858 Module, 0:LGIT ov8858 Module
#endif
...
// [LGE_UPDATE_S] [kipo.yun@lge.com] [2014-10-25] AT command added for main/sub camera to prevent mixed items
...
// [LGE_UPDATE_E] [kipo.yun@lge.com] [2014-10-25] AT command added for main/sub camera to prevent mixed items
/*******************************************************************************
*
********************************************************************************/
...
static unsigned int g_invokeSensorID; //[LGE_UPDATE][yonghwan.lym@lge.com][2015-01-02] Multiple EEPROM Suport
...
int iBurstWriteReg(u8 *pData, u32 bytes, u16 i2cId)
{
    ...
    int retry = 0;
        ...
        spin_lock(&kdsensor_drv_lock);
// LGE_CHANGE_S: [2014-12-04] kyunghun.oh@lge.com, Change i2c transfer mode into burst mode
...
#endif
        spin_unlock(&kdsensor_drv_lock);
// LGE_CHANGE_E: [2014-12-04] kyunghun.oh@lge.com, Change i2c transfer mode into burst mode
...
int kdSetDriver(unsigned int* pDrvIndex)
{
    ...
    for ( i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS ; i++ ) {
        ...
        if ( MAX_NUM_OF_SUPPORT_SENSOR > drvIdx[i] ) {
            ...
            //return sensor ID
            g_invokeSensorID = pSensorList[drvIdx[i]].SensorId; //[LGE_UPDATE][yonghwan.lym@lge.com][2015-01-02] Multiple EEPROM Suport
    ...
    if (g_pSensorFunc) {
        for ( i = KDIMGSENSOR_INVOKE_DRIVER_0 ; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS ; i++ ) {
            if(DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]){
                sensorID = g_invokeSensorID; //[LGE_UPDATE][yonghwan.lym@lge.com][2015-01-02] Multiple EEPROM Suport
...
inline static int  adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
    ...
    switch (pFeatureCtrl->FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE: //[LGE_UPDATE][yonghwan.lym@lge.com][2014-12-15] Get FOV from sensor for CTS
            //
...
//[LGE_UPDATE_S] [yonghwan.lym@lge.com] [2014-02-18] soc sensor Manual Flicker
...
static ssize_t Soc_antibanding_store(struct device* dev, struct device_attribute* attr, const char* buf, size_t n)
{
	...
	{
		case 0:
			soc_antibanding = 0; // 60HZ;
			break;
		case 1:
			soc_antibanding = 1; // 50HZ;
			break;
		default:
...
static DEVICE_ATTR(antibanding, /*S_IRUGO|S_IWUGO*/ 0664, NULL, Soc_antibanding_store);
...
#endif
//[LGE_UPDATE_E] [yonghwan.lym@lge.com] [2014-02-18] soc sensor Manual Flicker
...
bool is_sunny_ov8858_module()
{
    ...
    if(model_sel == 0) //LG-H502TV
        return 1; // Sunny ov8858 Module
    else
        return 0; // LGIT ov8858 Module
}
#endif
// [LGE_UPDATE_E][yonghwan.lym@lge.com][2015-01-29] LGIT and Sunny module check for Y90
/*******************************************************************************
