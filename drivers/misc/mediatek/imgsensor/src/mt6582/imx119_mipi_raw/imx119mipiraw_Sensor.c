...
#define IMX119MIPI_MaxGainIndex (87 + 1)																				 // Gain Index
//[LGE_UPDATE] [kipo.yun@lge.com] [2014-07-11] WHITE IMAGE ISSUE 1th index {0,0}-->{64,0}
kal_uint16 IMX119MIPI_sensorGainMapping[IMX119MIPI_MaxGainIndex][2] = {
    { 64 ,  0}, {64 ,  0}, {68 , 12}, {71 , 23}, {72 , 33}, { 74, 42}, {77 , 51}, { 80, 59}, { 84, 66}, { 87, 74}, { 91, 79},
...
static kal_uint16 IMX119MIPIReg2Gain(const kal_uint8 iReg)
{
    ...
    kal_uint8 iI;
    // Range: 1x to 8x
    for (iI = 1; iI < IMX119MIPI_MaxGainIndex; iI++) 
	{
	...
	//[LGE_UPDATE_S] [kipo.yun@lge.com] [2014-05-15] WBT_ISSUE
	//if(iI = 0)
	//	iI = 1;
	//[LGE_UPDATE_E] [kipo.yun@lge.com] [2014-05-15] WBT_ISSUE
...
static kal_uint8 IMX119MIPIGain2Reg(const kal_uint16 iGain)
{
    ...
    for (iI = 1; iI < (IMX119MIPI_MaxGainIndex-1); iI++) 
	{
		...
		case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
			{
				...
				*pHorFOV = (0x3D)<<16 | (0x3D); // 16:9 = 61, 4:3 = 61 // (61)<<16 | (61)
				*pVerFOV = (0x28)<<16 | (0x28); // 16:9 = 40, 4:3 = 40 // (40)<<16 | (40)
				*pFeatureParaLen = 8;
				break;
			}
