...
static imgsensor_info_struct imgsensor_info = {
    ...
    .normal_video = {
     ...
     .hs_video = {
        .pclk = 304000000,
        .linelength = 4008,                //record different mode's linelength
        .framelength = 628,            //record different mode's framelength
        .startx = 0,
        .starty = 0,
    ...
    .cap_delay_frame = 1,  //LGE_UPDATE [youmi.jun@lge.com] 2014/12/29, Flash timing tuning
    .pre_delay_frame = 3,
    .video_delay_frame = 3,
...
{{ 3264, 2448,    0,    0, 3264, 2448, 3264, 2448,    0,    0, 1632, 1224,     0,     0, 1632, 1224}, // Preview
 ...
 { 2560, 1920,  352,  264,  640,  480,  640,  480,    0,    0,  640,  480,     0,     0,  640,  480}, // hight speed video
 { 3264, 2448,  352,  504, 2560, 1440, 2560, 1440,    0,    0, 1280,  720,     0,     0, 1280,  720}};// slim video
...
// AG = 256/(B[7:0] + 32)
    // hi841╳０i﹌﹠oO╳▼使帚O8╳取﹌OA╳IAagain,(7 * 255 / 256)╳取﹌OEy╳０Ogain
    if(gain > 4080)
    {
        gain= 4080;
    }
    if(gain < 58)  // gain﹠iAreg╳０i﹌﹠oO﹠iEC255
    {
        gain = 58;
    }
    ...
    else
    {
        ...
        write_cmos_sensor(0x0205, 0);   // 8╳取﹌OA╳IAagain
...
//    MIPI speed(Mbps)        : 384Mbps x4lane
//    Frame Length            : 1256
//    Line Length         : 4008
//    Max Fps         : 30.20fps
//    Pixel order         : Green 1st (=GB)
...
write_cmos_sensor(0x0701, 0xd0);
write_cmos_sensor(0x6C04, 0x0c);
write_cmos_sensor(0x6C05, 0xb8);
write_cmos_sensor(0x6C06, 0x09);
write_cmos_sensor(0x6C07, 0x90);
write_cmos_sensor(0x6C08, 0x00);
write_cmos_sensor(0x6C01, 0x00);
write_cmos_sensor(0x6C09, 0x0f);
write_cmos_sensor(0x6C0A, 0xc0);
...
//    MIPI speed(Mbps)        : 760Mbps x4lane
//    Frame Length            : 2516
//    Line Length         : 4008
//    Max Fps         : 30.12fps
//    Pixel order         : Green 1st (=GB)
...
write_cmos_sensor(0x034B, 0xB7);
write_cmos_sensor(0x7407, 0x04);
write_cmos_sensor(0x7408, 0x01);
write_cmos_sensor(0x7409, 0x32);
...
// MIPI Setting
write_cmos_sensor(0x0800, 0x77);
write_cmos_sensor(0x0801, 0x28);
write_cmos_sensor(0x0802, 0x20);
write_cmos_sensor(0x0803, 0x3a);
write_cmos_sensor(0x0804, 0x3a);
write_cmos_sensor(0x0805, 0x28);
write_cmos_sensor(0x0806, 0xBF);
write_cmos_sensor(0x0807, 0x30);
...
write_cmos_sensor(0x6400, 0x01);
write_cmos_sensor(0x040C, 0x0A);
write_cmos_sensor(0x040D, 0x40);
...
write_cmos_sensor(0x6C00, 0x01);
write_cmos_sensor(0x0700, 0x00);
write_cmos_sensor(0x0701, 0x08);
write_cmos_sensor(0x6C04, 0x02);
write_cmos_sensor(0x6C05, 0x98);
write_cmos_sensor(0x6C06, 0x01);
write_cmos_sensor(0x6C07, 0xE2);
write_cmos_sensor(0x6C08, 0x00);
write_cmos_sensor(0x6C01, 0x00);
write_cmos_sensor(0x6C09, 0x00);
write_cmos_sensor(0x6C0A, 0x00);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
        ...
        case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
            ...
            break;
        //[LGE_UPDATE_S][youmi.jun@lge.com][2015-1-16] update FOV value
        case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
            {
                ...
                *pHorFOV = (0x3C)<<16 | (0x3C); // 16:9 = 60, 4:3 = 60 // (60)<<16 | (60)
                *pVerFOV = (0x31)<<16 | (0x31); // 16:9 = 49, 4:3 = 49 // (49)<<16 | (49)
                *feature_para_len = sizeof(uintptr_t) * 2;
                break;
            }
        //[LGE_UPDATE_E][youmi.jun@lge.com][2015-1-16] update FOV value
        default:
            break;
    }
