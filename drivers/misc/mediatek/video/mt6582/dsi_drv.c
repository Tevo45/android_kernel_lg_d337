...
#if defined(DB7436_DSI_VDO_FWVGA)  /* LGE_BSP_COMMON LGE_CHANGE_S 141010 jongwoo82.lee : MIXMODE enable for LGD incell */
//#define DISABLE_MIXMODE_CHANGE
//#define TEST_FOR_CHANGE_MODE
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141010 jongwoo82.lee : MIXMODE enable for LGD incell */
//#define TEST_FOR_CHANGE_MODE
...
#define WAIT_TOUCHENABLE_TIME       9
#define CNT_TIMER_FOR_MIXMODE       50
#define SKIP_FRAME_CNT_FOR_SUSPEND_MODE 5

static ktime_t dsi_single_frame_mode_timer_period;
struct hrtimer dsi_single_frame_mode_timer;
static int cnt_timer_for_dsi;
static int cnt_for_suspend_mode;
static DSI_STATUS_FOR_MIX_MODE dsi_incell_mix_mode_status;

static void delay_DSI_Start(void);
static void Start_MixMode(void);

static atomic_t dsi_wait_irq_count=ATOMIC_INIT(0);
#endif
...
#ifdef CONFIG_MIXMODE_FOR_INCELL
        if (lcm_params->dsi.mixmode_enable && (dsi_incell_mix_mode_status == DSI_INCELL_GPIO_TIMER_MODE || dsi_incell_mix_mode_status == DSI_INCELL_SEND_FRAME))
        {
		    delay_DSI_Start();
        }
#endif        
...
static void _WaitForEngineNotBusy(void)
{
	...
	irq_wait_cnt=atomic_inc_return(&dsi_wait_irq_count);
	if(dsi_log_on)
		printk("DSI %s: dsi_wait_irq_count++=%d\n", __func__,irq_wait_cnt);
    }
#endif
	timeOut = 200;
...
#ifdef CONFIG_MIXMODE_FOR_INCELL
#define CUST_EINT_DSI_TE_NUM        113
#ifndef GPIO_DSI_TE_PIN
#define GPIO_DSI_TE_PIN             (GPIO113 | 0x80000000)
#endif
#ifndef GPIO_DSI_TE_PIN_M_GPIO
#define GPIO_DSI_TE_PIN_M_GPIO      GPIO_MODE_00
#endif
#ifndef GPIO_DSI_TE_PIN_M_DSI_TE
#define GPIO_DSI_TE_PIN_M_DSI_TE    GPIO_MODE_01
#endif
#ifndef DSI_ENABLE_PWM_FOR_TE

static void DSI_TE_eint_func(void)
{
    ...
    #if !defined(DISABLE_MIXMODE_CHANGE)
...
static inline int DSI_video_TE_eint_setup(void)
{
    ...
    mt_set_gpio_pull_enable(GPIO_DSI_TE_PIN, GPIO_PULL_DISABLE); //To disable GPIO PULL.
...
void DSI_Enable_PWM_forTE(void)
{
	if(lcm_drv && lcm_drv->set_pwm_for_mix)
	    lcm_drv->set_pwm_for_mix(TRUE);
}

void DSI_Disable_PWM_forTE(void)
{
  	if(lcm_drv && lcm_drv->set_pwm_for_mix)
		lcm_drv->set_pwm_for_mix(FALSE);
}
#endif //DSI_ENABLE_PWM_FOR_TE

#endif //MIXMODE_FOR_INCELL
...
#ifdef CONFIG_MIXMODE_FOR_INCELL
#ifdef DSI_ENABLE_PWM_FOR_TE    
	#if defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141105 jongwoo82.lee : HW revision check */
    dsi_hw_rev = lge_get_board_revno();
    //printk("[LCD] DSI HW REV check : %d\n", dsi_hw_rev);
    if((lcm_params->dsi.mixmode_enable) && (dsi_hw_rev >= HW_REV_B))
    {
...
static enum hrtimer_restart _DSI_Send_Frame_without_TE_handler(struct hrtimer *timer)
{
    if (cnt_for_suspend_mode != 0)
    {
    	if(!_IsEngineBusy())
    	{
...
static void delay_DSI_Start(void)
{
	if(dsi_single_frame_mode_timer_period.tv64==0LL)
		printk("error: dsi_single_frame_mode_timer_period=0\n");
	//printk("start DSI_delay_hrtimer\n");
	hrtimer_start(&dsi_single_frame_mode_timer, dsi_single_frame_mode_timer_period, HRTIMER_MODE_REL);
}

#ifdef TEST_FOR_CHANGE_MODE

static struct task_struct *config_mixmode_task = NULL;

static int _MixModeKThread(void *data)
{
...
void DSI_change_mode(DSI_STATUS_FOR_MIX_MODE status)
{
    printk("DSI mode: new(%d), previous(%d) \n",status, dsi_incell_mix_mode_status);

    switch (status)
    {
        case DSI_INCELL_GPIO_TIMER_MODE:
#if defined(DSI_ENABLE_PWM_FOR_TE)  /* LGE_BSP_COMMON LGE_CHANGE_S 141027 jongwoo82.lee : mode not change for mix mode with PWM */
	#if defined(TARGET_MT6582_Y50)
            if(dsi_hw_rev >= HW_REV_B)
            {
            printk("dsi_drv, no need to change GPIO TIMER mode for PWM\n");
            return;        
            }
	#endif
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141027 jongwoo82.lee : mode not change for mix mode with PWM */
            if (dsi_incell_mix_mode_status == DSI_INCELL_MIX_MODE)
            {  
                OUTREGBIT(DSI_INT_ENABLE_REG,DSI_REG->DSI_INTEN,EXT_TE,1);
            ...
            else if (dsi_incell_mix_mode_status == DSI_INCELL_GPIO_TIMER_MODE)
            {
                cnt_timer_for_dsi = 0;
            }
            else
...
#ifdef DSI_ENABLE_PWM_FOR_TE            
	#if defined(TARGET_MT6582_Y50)
            else if((dsi_incell_mix_mode_status == DSI_INCELL_SUSPEND_MODE) && (dsi_hw_rev >= HW_REV_B))
	#else
            else if(dsi_incell_mix_mode_status == DSI_INCELL_SUSPEND_MODE)
	#endif
            {
        ...
        case DSI_INCELL_SEND_FRAME:
            if (lcm_params->dsi.send_frame_enable == TRUE)
            {                
                int cnt_sleep = 0;
			#if defined(TARGET_MT6582_P1S3G)
				dsi_incell_mix_mode_status = DSI_INCELL_SUSPEND_MODE;
                    #elif defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141217 jongwoo82.lee : LCD suspend reset issue fix by delay */
				dsi_incell_mix_mode_status = DSI_INCELL_SUSPEND_MODE;
                            //msleep(90);  // 5 frame duration --> no delay because during lcm suspend, delay applied
                    /* LGE_BSP_COMMON LGE_CHANGE_E 141217 jongwoo82.lee : LCD suspend reset issue fix by delay */
			#else
				DSI_Reset();
            ...
            #if defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141212 jongwoo82.lee : LCD shaking fix in Rev.A */
                if((lcm_params->dsi.mixmode_enable) && (dsi_hw_rev >= HW_REV_B))
            #else         
                if(lcm_params->dsi.mixmode_enable)
            #endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141212 jongwoo82.lee : LCD shaking fix in Rev.A */            
                {
                    OUTREGBIT(DSI_INT_ENABLE_REG,DSI_REG->DSI_INTEN,VM_DONE,1);            
                }           
                ...
                {   
                    ...
                    if (cnt_sleep++ > 100)
                    {
                        printk("Fail sending frame for suspend mode\n");
                        break;
                    }     
                }
#if defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141212 jongwoo82.lee : LCD shaking fix in Rev.A */
...
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141212 jongwoo82.lee : LCD shaking fix in Rev.A */
                {
                OUTREGBIT(DSI_INT_ENABLE_REG,DSI_REG->DSI_INTEN,VM_DONE,0);
                OUTREGBIT(DSI_MODE_CTRL_REG,DSI_REG->DSI_MODE_CTRL,FRM_MODE,0); 
...
DSI_STATUS_FOR_MIX_MODE DSI_getMode_Mix(void)
{
    return dsi_incell_mix_mode_status;
}

void DSI_EnableCMDDone(void)
{
    OUTREGBIT(DSI_INT_ENABLE_REG,DSI_REG->DSI_INTEN,CMD_DONE,1);
	atomic_inc_return(&dsi_wait_irq_count);
...
void DSI_PHY_clk_change(LCM_PARAMS *lcm_params)
{
    ...
    if(0!=data_Rate){//if lcm_params->dsi.PLL_CLOCK=0, use other method
        ...
        mdelay(1);
        OUTREGBIT(MIPITX_DSI_PLL_CHG_REG,DSI_PHY_REG->MIPITX_DSI_PLL_CHG, RG_DSI0_MPPLL_SDM_PCW_CHG,0);
        mdelay(1);
        OUTREGBIT(MIPITX_DSI_PLL_CHG_REG,DSI_PHY_REG->MIPITX_DSI_PLL_CHG, RG_DSI0_MPPLL_SDM_PCW_CHG,1);
...
void DSI_MixMode_Start(void)
{
    ...
    OUTREGBIT(DSI_INT_STATUS_REG,DSI_REG->DSI_INTEN,VM_CMD_DONE,0);
    OUTREGBIT(DSI_INT_ENABLE_REG,DSI_REG->DSI_INTEN,CMD_DONE,0);
    OUTREGBIT(DSI_INT_ENABLE_REG,DSI_REG->DSI_INTEN,EXT_TE,0);   
    OUTREGBIT(DSI_INT_ENABLE_REG,DSI_REG->DSI_INTEN,VM_DONE,0);    
...
//		pmod = 433.33;
	...
	OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_PHI_SEL,0x1);
        #if defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141127 jongwoo82.lee : LCD up/down shaking */
        OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_LPTX_IPLUS1,0x1); // +10% for P
        OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_LPTX_IPLUS2,0x0); // normal for N //0x1, +10% for N
        //OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_LPTX_IMINUS,0x1); // -10% for Both  
        #endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141127 jongwoo82.lee : LCD up/down shaking */
	...
	if(lcm_params->dsi.LANE_NUM > 0)
	{
        ...
        #if defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141127 jongwoo82.lee : LCD up/down shaking */
        OUTREGBIT(MIPITX_DSI0_DATA_LANE0_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE0,RG_DSI0_LNT0_LPTX_IPLUS1,0x1); // +10% for P
        OUTREGBIT(MIPITX_DSI0_DATA_LANE0_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE0,RG_DSI0_LNT0_LPTX_IPLUS2,0x0); // normal for N //0x1, +10% for N
        //OUTREGBIT(MIPITX_DSI0_DATA_LANE0_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE0,RG_DSI0_LNT0_LPTX_IMINUS,0x1); // -10% for Both      
        #endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141127 jongwoo82.lee : LCD up/down shaking */
	...
	if(lcm_params->dsi.LANE_NUM > 1)
	{
        ...
        #if defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141127 jongwoo82.lee : LCD up/down shaking */
        OUTREGBIT(MIPITX_DSI0_DATA_LANE1_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE1,RG_DSI0_LNT1_LPTX_IPLUS1,0x1); // +10% for P
        OUTREGBIT(MIPITX_DSI0_DATA_LANE1_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE1,RG_DSI0_LNT1_LPTX_IPLUS2,0x0); // normal for N //0x1, +10% for N
        //OUTREGBIT(MIPITX_DSI0_DATA_LANE1_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE1,RG_DSI0_LNT1_LPTX_IMINUS,0x1); // -10% for Both        
        #endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141127 jongwoo82.lee : LCD up/down shaking */
...
#define NS_TO_CYCLE(n, c)	((n) / (c))
   ...
   #if defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141203 jongwoo82.lee : physical timing optimization */
   timcon0.HS_TRAIL = ((hs_trail_m > hs_trail_n) ? hs_trail_m : hs_trail_n) + 0x05;
   #else
   timcon0.HS_TRAIL = ((hs_trail_m > hs_trail_n) ? hs_trail_m : hs_trail_n) + 0x0a;
   #endif  /* LGE_BSP_COMMON LGE_CHANGE_E 141203 jongwoo82.lee : physical timing optimization */
...
#if defined(TARGET_MT6582_Y50)  /* LGE_BSP_COMMON LGE_CHANGE_S 141203 jongwoo82.lee : physical timing optimization */
    timcon2.CLK_TRAIL    = ((lcm_params->dsi.CLK_TRAIL == 0) ? NS_TO_CYCLE(0x64, cycle_time) : lcm_params->dsi.CLK_TRAIL);
#else
...
//	_WaitForEngineNotBusy();
	if(!s_isDsiPowerOn)
	{
		printk("[DSI Error0] DSI has been suspended, but still operate DSI HW\n");
		return;
	}
	if(((INREG32(&DSI_PHY_REG->MIPITX_DSI_PLL_CON0)) & 0x01) != 0x1)
	{
		printk("[DSI Error1] MIPI TX has been OFF, but still operate DSI HW\n");
		return;
	}
	if (0 != DSI_REG->DSI_MODE_CTRL.MODE){//not in cmd mode
		DSI_VM_CMD_CON_REG vm_cmdq;
