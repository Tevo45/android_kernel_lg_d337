...
static struct i2c_board_info kd_lens_dev __initdata = { I2C_BOARD_INFO("DW9714AF", 0x19) };
...
#define DW9714AFDB printk //[LGE_UPDATE][yonghwan.lym@lge.com][2014-12-16] af kernel log enable
#else
#define DW9714AFDB(x, ...)
#endif
...
static int g_s4DW9714AF_RELEASE;////[LGE_UPDATE][kyunghun.oh@lge.com][2015-01-07] DECREASE_AF_TIC_NOISE
...
#define THRESHOLD 5
//[LGE_UPDATE_S][kyunghun.oh@lge.com][2015-01-07] DECREASE_AF_TIC_NOISE
static int breakDownSteps(int max_step, int interval, int CurrentPos, int TargetPos)
{
	unsigned short tmpTargetPos;
	DW9714AFDB("[DW9714AF] break down steps from=%d, to=%d \n", CurrentPos, TargetPos);
	...
	while( max_step >= THRESHOLD )
	{
		while( abs(CurrentPos - TargetPos) > max_step ){
			...
			DW9714AFDB("tmpTargetPos=%d\n", tmpTargetPos);
			if(s4DW9714AF_WriteReg(tmpTargetPos) == 0)
			{
				CurrentPos = tmpTargetPos;
				DW9714AFDB("CurrentPos=%d\n", CurrentPos);
			}
			else
			{
	...
	if(s4DW9714AF_WriteReg(TargetPos) == 0)
	{
		CurrentPos = TargetPos;
	}
	else
	...
	DW9714AFDB("Leave breakdown\n");
	return 0;
}
//[LGE_UPDATE_E][kyunghun.oh@lge.com][2015-01-07] DECREASE_AF_TIC_NOISE
#ifdef LensdrvCM3
...
inline static int moveDW9714AF(unsigned long a_u4Position)
{
	...
	g_i4MotorStatus = 0;
	spin_unlock(&g_DW9714AF_SpinLock);
			//[LGE_UPDATE_S][yonghwan.lym@lge.com][2014-06-10] AF control : Slew Rate and Step
			if( (g_i4Dir == -1) && (g_u4CurrPosition!=g_u4TargetPosition) && (g_u4TargetPosition<250)){
				breakDownSteps(40, 10, g_u4CurrPosition, (g_u4TargetPosition));
			}
			else
			{
             ...
             //[LGE_UPDATE_E][yonghwan.lym@lge.com][2014-06-10] AF control : Slew Rate and Step
			}
	//[LGE_UPDATE_E][kyunghun.oh@lge.com][2015-01-07] DECREASE_AF_TIC_NOISE
	return 0;
}
...
/* ////////////////////////////////////////////////////////////// */
static long DW9714AF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	long i4RetValue = 0;
...
static int DW9714AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstDW9714AF_I2Cclient->addr = g_pstDW9714AF_I2Cclient->addr >> 1;
