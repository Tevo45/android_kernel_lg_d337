...
#define K2HH_HIGH_RESOLUTION                 0x80

#define AXISDATA_REG                        0x28
#define WHOAMI_K2DH_ACC                    0x33    /*  Expctd content for WAI    */ 
#define K2HH_WHOAMI_K2DH_ACC              0x41 //for K2HH

#define IF_ADD_INC						0x04  //for K2HH
...
//for K2HH
#define K2HH_ODR_POWERDOWN_MODE           0x00  /* Power Down Mode */
#define K2HH_ODR10              0x10  /* 10Hz output data rate */
#define K2HH_ODR50              0x02  /* 50Hz output data rate */
#define K2HH_ODR100             0x30  /* 100Hz output data rate */
#define K2HH_ODR200             0x40  /* 200Hz output data rate */  // default speed
#define K2HH_ODR400             0x50  /* 400Hz output data rate */
#define K2HH_ODR800             0x60    /* 800Hz output data rate */
...
//for K2HH
#define K2HH_I2C_AUTO_INCREMENT    0x00
...
//for K2HH
#define K2HH_SELF_TEST_2G_MAX_LSB	(24576)
#define K2HH_SELF_TEST_2G_MIN_LSB	(1146)

#define K2DH_SELF_TEST_2G_MAX_LSB        (360)
#define K2DH_SELF_TEST_2G_MIN_LSB        (17)
#define TESTLIMIT_XY                  (130)  //(175)
#define TESTLIMIT_Z_USL_LSB           (635) //(1270)
#define TESTLIMIT_Z_LSL_LSB           (390) //(778)
...
static struct i2c_board_info __initdata k2hh_i2c_info ={ I2C_BOARD_INFO(K2DH_ACC_DEV_NAME, K2HH_ACC_I2C_SAD_L)}; //for K2HH
...
#if defined(CALIBRATION_TO_FILE)
static int k2dh_calibration_read(int *cal_read);
static int k2dh_calibration_save(int *cal);
...
#if 1 /* Check Each Axis Data */
static u8 ReservedRegData[12] = {0}; 
static int Same_XData_Count = 0;
static int Same_YData_Count = 0;
static int Same_ZData_Count = 0;
#endif
...
static int k2dh_CheckDeviceID(struct i2c_client *client)
{
   ...
   //for K2HH
    if(K2HH)
    {
	    if(databuf[0] != K2HH_WHOAMI_K2DH_ACC)
	    {
	        GSE_ERR("device unknown. Expected: 0x%x, Replies: 0x%x\n", K2HH_WHOAMI_K2DH_ACC, databuf[0]);
...
/* Check Each Axis Data : Start */
static int k2dh_StoreHiddenReg(struct i2c_client *client)
{
   ...
   for(i=0;i<12;i++)
   	{
      databuf_reg[0] = i;
      res = k2dh_acc_i2c_read(client, databuf_reg, 1);
      ...
      if(res != K2DH_SUCCESS)
        {
            ...
            res = k2dh_acc_i2c_read(client, databuf_reg, 1);
              if(res != K2DH_SUCCESS){
                     printk("[gsensor] k2dh_StoreHiddenReg : Read Error Reg = 0X%x, data=%d\n", i, databuf_reg[0]);
              	}
              else{
                     printk("[gsensor] k2dh_StoreHiddenReg : Reg = 0X%x, data=%d\n",i, databuf_reg[0]);
...
static int k2dh_CheckHiddenReg(struct i2c_client *client)
{
    u8 databuf_reg[2] = {0,0};
    int res = -1;
    int i=0;
   ...
   for(i=0;i<12;i++)
   	{
         ...
         if(res != K2DH_SUCCESS)
           {
             printk("[gsensor] k2dh_CheckHiddenReg : Read Error Reg = 0X%x, data=%d ...................\n", i, databuf_reg[0]);
           }
         else
          {
           if(databuf_reg[0]==ReservedRegData[i])
            printk("[gsensor] k2dh_CheckHiddenReg Same : Reg = 0X%x, data=%d, ReservedRegData=%d ...................\n",i, databuf_reg[0],ReservedRegData[i]);
           else
            printk("[gsensor] k2dh_CheckHiddenReg Diff : Reg = 0X%x, data=%d, ReservedRegData=%d...................\n",i, databuf_reg[0],ReservedRegData[i]);
          }
   	  	}
    }
...
static int k2dh_HiddenReg_Recovery(struct i2c_client *client)
{
    u8 databuf_reg[2] = {0,0};
    u8 write_buf[2]={0,0};
    int res = -1;
    ...
    memset(databuf_reg, 0, sizeof(u8)*2);
    memset(write_buf, 0, sizeof(u8)*2);
   ...
   for(i=0;i<12;i++)
   	{
         ...
         if(res != K2DH_SUCCESS)
           {
             printk("[gsensor] k2dh_HiddenReg_Recovery : Read Error Reg = 0X%x, data=%d ...................!!!\n",i,databuf_reg[0]);
           }
          else
           {
              if(databuf_reg[0] !=ReservedRegData[i])
                {
                  printk("[gsensor] k2dh_HiddenReg_Recovery : Change Reg = 0X%x, data=%d to ReservedRegData=%d ...................!!!\n",i,databuf_reg[0],ReservedRegData[i]);             
                  write_buf[0] = i;
                  write_buf[1] = ReservedRegData[i];
                  err = k2dh_acc_i2c_write(client, write_buf,1);  
               }
         	 }
   	   	}
    }
...
{  // bwrate : ODRxxx
   ...
   if(K2HH) //for K2HH
   	{
        new_val = bwrate | K2DH_ACC_ENABLE_ALL_AXES | K2HH_HIGH_RESOLUTION;
    	mask = K2HH_ODR_MSK | K2DH_ACC_ENABLE_ALL_AXES_MSK  | K2HH_HIGH_RESOLUTION;
   	}
...
{  // power mode controled by operating Bandwidth(ODR-Hz)
    ...
    {
        GSE_LOG("Sensor power status is newest!(same power set)\n");
        return K2DH_SUCCESS;
    }
    ...
    if(K2HH)
    	{
		    if(enable == TRUE)
		    {
		        res = k2dh_SetBWRate(client, K2HH_ODR400);  // operating mode as 400HZ
		    }
		    else
		    {
		        res = k2dh_SetBWRate(client, K2HH_ODR_POWERDOWN_MODE);  // power down mode
		    }
    	}
    else
    	{
		    if(enable == TRUE)
		    {
		        res = k2dh_SetBWRate(client, ODR400);  // operating mode as 400HZ
		    }
		    else
		    {
...
{  // dataformat : K2DH_ACC_G_2G . K2DH_ACC_G_4G. K2DH_ACC_G_8G. K2DH_ACC_G_16G
    ...
    if(K2HH)
      mask = K2DH_ACC_FS_MASK | IF_ADD_INC | AAF_BW_100Hz;
    else
      mask = K2DH_ACC_FS_MASK | HIGH_RESOLUTION;
    ...
    else
    {
        ...
        if(K2HH)
        new_val = dataformat | IF_ADD_INC | AAF_BW_100Hz;
        else
...
static int k2hh_init_client(struct i2c_client *client)
{
    ...
    buf[0] = (K2HH_I2C_AUTO_INCREMENT | TT_THS);
    buf[1] = 0x00;
    buf[2] = 0x00;
    buf[3] = 0x00;
    buf[4] = 0x00;
    ...
    buf[0] = (K2HH_I2C_AUTO_INCREMENT | INT_THS1);
    buf[1] = 0x00;
    buf[2] = 0x00;
    err = k2dh_acc_i2c_write(client, buf, 2);
    if (err < 0)
    ...
    buf[0] = (K2HH_I2C_AUTO_INCREMENT | INT_THS2);
    buf[1] = 0x00;
    buf[2] = 0x00;
    err = k2dh_acc_i2c_write(client, buf, 2);
    if (err < 0)
    ...
    buf[0] = (K2HH_I2C_AUTO_INCREMENT | CTRL_REG2);
    buf[1] = 0x00; // for CTRL_REG2 LPF cut-off= [ODR/50]Hz=[400/50]=8Hz
    //buf[1] = 0x20; // for CTRL_REG2 LPF cut-off= [ODR/100]Hz
    //buf[1] = 0x40;// for CTRL_REG2 LPF cut-off= [ODR/9]Hz
    //buf[1] = 0x60; // for CTRL_REG2 LPF cut-off=[ODR/400]Hz
    buf[2] = 0x00;
    buf[3] = 0x00;
    buf[4] = 0x00;
    buf[5] = 0x00;
    ...
    if (err < 0)
        ...
        GSE_LOG("k2hh_init_client : hw init done\n");
        return K2DH_SUCCESS;
   ...
   if(K2HH) //for K2HH
    {
    	...
    	res = k2dh_SetBWRate(client, K2HH_ODR400);
		if(res != K2DH_SUCCESS )
		{
		    return res;
		}
   ...
   else
    {
		...
		res = k2dh_SetBWRate(client, ODR400);
		if(res != K2DH_SUCCESS )
		{
		    return res;
		}
    ...
    if(K2HH){
    res = k2dh_SetDataFormat(client, K2HH_ACC_G_4G);//changed to +/-4g setting for LOS
	}
    else{
    res = k2dh_SetDataFormat(client, K2DH_ACC_G_4G);//changed to +/-4g setting for LOS
	}
    if(res != K2DH_SUCCESS)
    {
        return res;
    ...
    if(K2HH==0)
    k2dh_StoreHiddenReg(client); /* For Check Each Axis Data */
...
/*----------------------------------------------------------------------------*/
#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 140326 jongwoo82.lee :  read data error check&recovery */
static s16 prev_data[K2DH_AXES_NUM] = {0,0,0};
static s16 check_cnt = 0;
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 140326 jongwoo82.lee : read data error check&recovery */
...
static int k2dh_ReadData(struct i2c_client *client, s16 data[K2DH_AXES_NUM])
{
   ...
   if(K2HH) //for K2HH
    acc_data[0] = (K2HH_I2C_AUTO_INCREMENT | AXISDATA_REG);
   else
...
#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 140327 jongwoo82.lee : read data error check&recovery */
    if(K2HH==0)
  {
   /* Check Each Axis Data : Start */
       if(prev_data[K2DH_AXIS_X] == data[K2DH_AXIS_X])
          Same_XData_Count++;
       else
          Same_XData_Count	=0;
  
       if(prev_data[K2DH_AXIS_Y] == data[K2DH_AXIS_Y])
          Same_YData_Count++;
       else
       	  Same_YData_Count=0;

       if(prev_data[K2DH_AXIS_Z] == data[K2DH_AXIS_Z])
          Same_ZData_Count++;
       else
       	  Same_ZData_Count=0;

      if((Same_XData_Count > 10) || (Same_YData_Count > 10) || (Same_ZData_Count > 10))
      	{
      	  if(Same_XData_Count > 10)
           GSE_ERR("READ X DATA SAME CASE : X Data = %d\n", data[K2DH_AXIS_X]);

      	  if(Same_YData_Count > 10)
           GSE_ERR("READ Y DATA SAME CASE : Y Data = %d\n", data[K2DH_AXIS_Y]);

      	  if(Same_ZData_Count > 10)
           GSE_ERR("READ Z DATA SAME CASE : Z Data = %d\n", data[K2DH_AXIS_Z]);
     	 
		   k2dh_HiddenReg_Recovery(client);
...
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 140327 jongwoo82.lee : read data error check&recovery */
...
static int k2dh_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
    ...
    else
    {
        ...
        if((data_read_count%500)==0)
            printk("gsensor data: debug count = %d, x=%d, y=%d, z=%d!\n",data_read_count,acc[K2DH_AXIS_X], acc[K2DH_AXIS_Y], acc[K2DH_AXIS_Z]);
...
static int k2dh_do_calibration(void)
{
      ...
      /* Sensor stable time after power on  */
       msleep ( 300 ); 
        ...
        {
            obj->cali_sw[K2DH_AXIS_Z] = (sum[K2DH_AXIS_Z] / CALIBRATION_DATA_AMOUNT) + 512;    //changed for K2HH 4g setting    // K2DH(12bit) 1024 +- 226
        }
        else
        {
            obj->cali_sw[K2DH_AXIS_Z] = (sum[K2DH_AXIS_Z] / CALIBRATION_DATA_AMOUNT) - 512;    //changed for K2HH 4g setting     // K2DH(12bit) 1024 +- 226
...
#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 140326 jongwoo82.lee : Selftest */
static int k2dh_get_selftest(struct i2c_client *client)
{
    ...
    for(val = 0, i = 0; i < 3; i++)
    {
        ...
        if((K2DH_SELF_TEST_2G_MIN_LSB > ST[i]) || (ST[i] > K2DH_SELF_TEST_2G_MAX_LSB))
        {
...
//initialize the init setting after selftest
if(err = k2dh_acc_hw_init(client, 1))
    {
...
static int k2hh_get_selftest(struct i2c_client *client)
{
    ...
    x[0] = (K2HH_I2C_AUTO_INCREMENT | CTRL_REG4);
	x[1] = 0x24;//0x04
	x[2] = 0x00;
	x[3] = 0x00;
    k2dh_acc_i2c_write(client, x, 3);
    ...
    x[0] = (K2HH_I2C_AUTO_INCREMENT | AXISDATA_REG);
    k2dh_acc_i2c_read(client, x, 6);
    ...
    for(val = 0, i = 0; i < 3; i++)
    {
        ...
        // range compare of the self test
        if((K2HH_SELF_TEST_2G_MIN_LSB > ST[i]) || (ST[i] > K2HH_SELF_TEST_2G_MAX_LSB))
        {
            GSE_ERR("[SELFTEST] ST[%d] : Out of range!! (%d)\n", i, ST[i]);
            val = -1;
        }
...
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 140326 jongwoo82.lee : Selftest */
...
static int k2dh_set_mode(struct i2c_client *client, unsigned char mode)
{
    ...
    if(!K2HH) //for K2HH
   {
   	...
   	{
        comres = k2dh_acc_register_update(client, (u8 *) data, K2HH_HR_REG, K2HH_HR_MSK, HR << K2HH_HR_POS);
        if(comres != K2DH_SUCCESS)
        {
            return comres;
        }
  ...
  if(!K2HH) //for K2HH
    ...
    else
    {
      if(!K2HH) //for K2HH  
       ...
       {
        LPen =0;
        comres = k2dh_acc_register_read(client, data, K2HH_HR_REG);
       }
...
static int k2dh_set_bandwidth(struct i2c_client *client, unsigned char bandwidth)
{
   ...
   if(K2HH)
    comres = k2dh_acc_register_update(client, (u8 *) data, K2HH_ODR_REG, K2HH_ODR_MSK, bandwidth << K2HH_ODR_POS);
   else
...
static int k2dh_get_bandwidth(struct i2c_client *client, unsigned char *bandwidth)
{
    ...
    if(K2HH)
    comres = k2dh_acc_register_read(client, data, K2HH_ODR_REG);
    else
...
static int k2dh_calibration_read(int *cal_read)
{
   ...
   for(i=0;i<6;i++)
   {
        ...
        GSE_LOG("k2dh_calibration_read : cal_read[%d]=%d\n",i,cal_read[i]);
   	}
   sys_close(fd);
   set_fs(old_fs);
   GSE_LOG("k2dh_calibration_read Done.\n");
...
#endif
   GSE_LOG("k2dh_calibration_save Done.\n");
...
#if defined(CALIBRATION_TO_FILE)
   if(k2dh_calibration_read(cal_backup) == 0)
...
{        
		...
		{
		   #if 1 //motion sensor cal backup
		        cali[3] = obj->cali_sw[0];
                        cali[4] = obj->cali_sw[1];
                        cali[5] = obj->cali_sw[2];
		   #endif
            ...
            if(k2dh_runCalibration() == K2DH_SUCCESS)
			{
				...
				#if defined(CALIBRATION_TO_FILE)
					if(k2dh_calibration_save(cali) == K2DH_SUCCESS)
...
static ssize_t k2dh_runCalibration_show(struct device_driver *ddri, char *buf)
{
	...
	printk("%s: fast_calib_rslt=%d selfCalibration=%d \n",__func__,atomic_read(&obj->fast_calib_rslt),selfCalibration);
	return sprintf(buf, "%d\n", selfCalibration);
}
    ...
    switch (command)
    {
...
#if 1  /* LGE_BSP_COMMON LGE_CHANGE_S 140326 jongwoo82.lee : ODR fix ad 200Hz */
               if(K2HH)
                sample_delay = K2HH_ODR400;  // 2.5ms
                else
                sample_delay = ODR400;  // 2.5ms
...
#endif  /* LGE_BSP_COMMON LGE_CHANGE_E 140326 jongwoo82.lee : ODR fix ad 200Hz */
        ...
        case SENSOR_ENABLE:
            ...
            else
            {
                ...
                {
                    GSE_LOG("Gsensor device have updated!, (same)power: %d\n", sensor_power);
                }
                else
                {
...
static long k2dh_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        ...
        case GSENSOR_IOCTL_SET_CALI:
			...
			#if defined(CALIBRATION_TO_FILE)
				if(make_cal_data_file() == K2DH_SUCCESS)
				{
	            err = k2dh_calibration_read(cali);
				if(err!=0){
...
static int k2dh_probe(struct platform_device *pdev)
{
    ...
    GSE_FUN();
#if defined(TARGET_S4)
     k2dh_power(hw, 1);  // no need to power on because of using always on power for accelerometer
     msleep ( 10 );         // no need sleep
#endif
