	...
	for (;;) {
		...
		if (loc_wk_wdt && loc_wk_wdt->ready && g_enable) {
			...
			if (wk_tsk[cpu]->pid == current->pid) {
				...
				if (local_bit ==8 || local_bit == wk_check_kick_bit()) {
...
static int start_kicker(void)
{
    ...
    wk_cpu_update_bit_flag(0, 1); //MTK patch ALPS01913721 kernel crash after downloading by LGE UP with refurbish option
	for (i = 0; i < CPU_NR; i++) {
