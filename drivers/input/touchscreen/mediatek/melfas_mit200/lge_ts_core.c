...
#ifdef IS_MTK //for file access
#include <linux/syscalls.h>
#include <linux/uaccess.h> 
#include <asm/uaccess.h>

unsigned char *my_image_fw;
#endif

#include "L0M45P1CY5_30_21_bin.h"          
#include "L0M45P2_30_01_bin.h" //rev.a
...
#else
	if ( atomic_read(&key_lock_status) != TRUE )
 ...
 */
int touch_ic_pin_check(struct touch_platform_data *pdata, int *pinstate)
{
	TOUCH_TRACE_FUNC();

	*(pinstate+0) = mt_get_gpio_pull_select(pdata->sda_pin);
	*(pinstate+1) = mt_get_gpio_pull_select(pdata->scl_pin);
	*(pinstate+2) = mt_get_gpio_in(pdata->int_pin);

	TOUCH_ERR_MSG("pin state [sda:%d, scl:%d, int:%d]\n", *(pinstate+0), *(pinstate+1),  *(pinstate+2) );
...
static int touch_ic_init(struct lge_touch_data *ts)
{
...
#else
	if (touch_ic_pin_check(ts_pdata, pinstate) == -EIO)
	{		
		goto err_out_retry;
	}
#endif
	...
	//ticklewind.kim_S:141229
	ts->pdata->lpwg_mode_old = LPWG_NONE;
	//ticklewind.kim_E:141229
	return 0;
...
static int touch_request_firmware(struct lge_touch_data *ts)
{
	...
	int ret = 0;
	int hw_rev = lge_get_board_revno();
	UINT8* fw_binary = MELFAS_binary;
	size_t fw_length = (size_t)sizeof(MELFAS_binary);

	TOUCH_INFO_MSG("HW_REV : %d \n", hw_rev);

#if defined(TARGET_MT6582_Y50)
	if (hw_rev == HW_REV_A)
	{
		fw_binary = MELFAS_binary_A;
		fw_length = (size_t)sizeof(MELFAS_binary_A);
	}
#endif
	...
	{
//		TOUCH_INFO_MSG("MELFAS_binary = 0x%X, (size_t)sizeof(MELFAS_binary) = 0x%X. the data present hex unit", MELFAS_binary, (size_t)sizeof(MELFAS_binary));

		if ( ts->fw_info.fw == NULL )
		{
			TOUCH_ERR_MSG("ts->fw_info.fw isn't \n");
			ts->fw_info.fw =  kzalloc(sizeof(struct firmware), GFP_KERNEL);
			if (!ts->fw_info.fw) {
				TOUCH_ERR_MSG("kmalloc(struct firmware) failed\n");
				goto out;
			}
		}
			
		if (ts->fw_info.path[0] == NULL )
		{
			TOUCH_ERR_MSG("ts->fw_info.path[0] isn't \n");
	
			ts->fw_info.fw->data = (u8*) fw_binary;
			ts->fw_info.fw->size = fw_length;
		}

		if ( (u16)ts->fw_info.fw->size != (u16)fw_length )
		{
			TOUCH_ERR_MSG("MELFAS_binary_nLength incorrect \n");		
			ret = 1;
			goto out;
		}
//		else
//			TOUCH_INFO_MSG(" ts->fw_info.fw = 0x%X, ts->fw_info.fw->size = 0x%X. the data present hex unit", ts->fw_info.fw->data, ts->fw_info.fw->size);
...
static void touch_fw_upgrade_func(struct work_struct *work_fw_upgrade)
{
	...
	if (touch_drv->fw_upgrade(ts->client, &ts->fw_info) < 0) { //start touch f/w update
		...
		memset(ts->fw_info.path, 0x00, sizeof(ts->fw_info.path));
...
static int touch_thread_irq_handler ( void * dev_id )
{
	...
	do
	{
		...
		if (unlikely(atomic_read(&ts->device_init) != 1) && atomic_read(&dev_state) == DEV_RESUME || ts->sd_status == 1) {
			TOUCH_ERR_MSG("Touch ic is not ready for operation.");
			goto exit_work;
		}
...
#else
		if ( client->irq != CUST_EINT_TOUCH_PANEL_NUM )
...
static int touch_parse_dt(struct device *dev, struct touch_platform_data *pdata)
{
	...
	struct touch_dt_map map[] = {
		...
		{ "lge,sda-gpio", &pdata->sda_pin, DT_GPIO, GPIO_TOUCH_I2C_SDA },
...
//case DT_STRING:
		...
		strcpy(pdata->fw_product , "L0M45P1CY5");
...
{	
	if ( key_lock )	
		touch_disable(key_lock);
	else
		touch_enable(!key_lock);
	atomic_set(&key_lock_status, key_lock);
}
...
static int touch_fb_resume(struct device *device)
{
	...
	TOUCH_INFO_MSG("key_lock_status : %d\n", atomic_read(&key_lock_status) );
	ts->ic_error_cnt =0;
...
#ifdef IS_MTK
static int FirmwareLoad(struct lge_touch_data *ts, const char* fw_path)
{
	TOUCH_TRACE_FUNC();
	...
	if(unlikely(fw_path[0] != 0)) {
		...
		ts->fw_info.fw->size = fw_bin_stat.st_size;		
		TOUCH_INFO_MSG("before Touch FW image read %ld bytes \n", fw_bin_stat.st_size);
		
		my_image_fw = kzalloc(sizeof(char) * (ts->fw_info.fw->size), GFP_KERNEL);
		if (my_image_fw == NULL) {
		...
		read_bytes = sys_read(fd, (char __user *)my_image_fw, ts->fw_info.fw->size);

		ts->fw_info.fw->data = my_image_fw; 

		TOUCH_INFO_MSG("Touch FW image read %ld bytes from %s\n", read_bytes, fw_path);

		target[0] = my_image_fw[0xFFFA];
		target[1] = my_image_fw[0xFFFB];

		TOUCH_INFO_MSG("my_image_fw File Version : %X.%02X\n", target[0], target[1]);
		}
fw_mem_alloc_fail:
	sys_close(fd);
read_fail:
...
static ssize_t fw_upgrade_store(struct lge_touch_data *ts, const char *buf, size_t count, bool forceupgrade)
{
	...
	} else if (strncmp(&cmd[strlen(cmd) - 3], ".fw", 3) == 0 || strncmp(&cmd[strlen(cmd) - 3], ".FW", 3) == 0 ||  strncmp(&cmd[strlen(cmd) - 4], ".bin", 4) == 0 ||  strncmp(&cmd[strlen(cmd) - 4], ".img", 4) == 0) {
		TOUCH_INFO_MSG("F/W File : %s\n",cmd);
		snprintf(ts->fw_info.path, NAME_BUFFER_SIZE, "%s", cmd);

#ifdef IS_MTK
//ticklewind.kim@lge.com
		ret = FirmwareLoad(ts, ts->fw_info.path);
		if(ret < 0) {
			TOUCH_ERR_MSG("Firmware Load Fail!!!\n");
		} else {
			TOUCH_INFO_MSG("Firmware Load Complete\n");
		}
#endif
...
#ifdef IS_MTK
static ssize_t knock_on_type_show(struct lge_touch_data *ts, char *buf)
{
	int ret = 0;
	ret = sprintf(buf, "%d", 1);
	return ret;
...
static LGE_TOUCH_ATTR(fw_ver, S_IRUGO | S_IWUSR, testmode_version_show, NULL);
...
#ifdef IS_MTK
static LGE_TOUCH_ATTR(knock_on_type, S_IRUGO, knock_on_type_show, NULL);
#endif
#if defined(TOUCH_USE_DSV)
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	atomic_set(&ts->device_init, 0);
	atomic_set(&dev_state,DEV_RESUME_ENABLE);
	atomic_set(&key_lock_status, DO_NOT_ANYTHING);
	/* Power on */
...
MODULE_DESCRIPTION ( "Customizing MIT 200 Touchscreen Driver for MTK platform" );
