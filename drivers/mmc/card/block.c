	...
	struct mmc_ioc_cmd __user *ic_ptr)
{
	...
	if(cmd.opcode == MMC_FFU_DOWNLOAD_OP) {
		printk(KERN_CRIT "[LGE][FFU] opcode = MMC_FFU_DOWNLOAD_OP\n");
		err = mmc_ffu_download(card, &cmd, idata->buf, idata->buf_bytes);
		goto cmd_rel_host;
	}
	if(cmd.opcode == MMC_FFU_INSTALL_OP) {
		printk(KERN_CRIT "[LGE][FFU] opcode = MMC_FFU_INSTALL_OP\n");
		err = mmc_ffu_install(card);
		goto cmd_rel_host;
	}
	if(cmd.opcode == MMC_FFU_MID_OP) {
		printk(KERN_CRIT "[LGE][FFU] opcode = MMC_FFU_MID_OP, [cid : %u]\n", card->cid.manfid);
	...
	if(cmd.opcode == MMC_FFU_PNM_OP) {
		printk(KERN_CRIT "[LGE][FFU] opcode = MMC_FFU_PNM_OP, [pnm : %s]\n", card->cid.prod_name);
		if (copy_to_user((void __user *)(unsigned long) idata->ic.data_ptr,
...
static int mmc_blk_issue_flush(struct mmc_queue *mq, struct request *req)
{
	...
	if (ret == -ETIMEDOUT) {
		pr_err("%s:%s requeue flush request after timeout", mmc_hostname(card->host),__func__);
	 ...
	 */
		...
		if ((card->host->caps & MMC_CAP_NONREMOVABLE) && timeout_count > 3)		// Only for eMMC (NONREMOVABLE)
		{
			do
			{
				...
				pr_err("%s:%s: mmc_flush_cache() fails:%d:%d, call mmc_blk_reset() again:%d.\n", 
						mmc_hostname(card->host), __func__, ret, err, reset_count);
			} while (err<0 && reset_count < 3);
			if (err < 0)
			{
				pr_err("%s:%s: retry mmc_blk_reset() fails three times. abort it otherwise phone is hang:%d.\n", 
...
static int mmc_blk_reset_if_hang(struct mmc_queue *mq, struct mmc_card *card)
{
	...
	if (card->host->caps & MMC_CAP_NONREMOVABLE)		// Only for eMMC (NONREMOVABLE)
	{
		do
		{
			reset_count++;
			err = mmc_blk_reset(md, card->host, 0);				// mmc_blk_reset() return zero if re-init is done successfully. do force reset.
			pr_err("%s:%s: mmc_blk_issue_rw_rq() fail:%d, call mmc_blk_reset() again:%d.\n", 
